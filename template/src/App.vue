<template>
  <div id="app">
    <mt-tab-container v-model="selected" :style="{height: containerHeight + 'px'}">
      <mt-tab-container-item id="home">
        <home></home>
      </mt-tab-container-item>
      <mt-tab-container-item id="community">
        <community></community>
      </mt-tab-container-item>
      <mt-tab-container-item id="profile">
        <profile></profile>
      </mt-tab-container-item>
    </mt-tab-container>

    <mt-tabbar v-model="selected" fixed class="app-bottom-tabber" :style="{paddingBottom: $paddingBottom + 'px'}">
      <mt-tab-item id="home">
        <img slot="icon" :src="icon">
        首页
      </mt-tab-item>
      <mt-tab-item id="community">
        <img slot="icon" :src="icon">
        社区
      </mt-tab-item>
      <mt-tab-item id="profile">
        <img slot="icon" :src="icon">
        我的
      </mt-tab-item>
    </mt-tabbar>

    <!-- 把需要缓存页面的name放进include -->
    <keep-alive :include="[]">
      <router-view />
    </keep-alive>
  </div>
</template>

<script>
import Home from 'pages/home'
import Community from 'pages/community'
import Profile from 'pages/Profile'

export default {
  name: 'App',
  data() {
    return {
      icon: 'data:image/png;base64,',
      selected: 'home',
      containerHeight: ''
    }
  },
  created() {
    this.containerHeight = document.body.offsetHeight - this.$tabHeight - this.$paddingBottom
  },
  components: {
    Home, Community, Profile
  }
}
</script>

<style lang="stylus">
@import '~assets/stylus/variable.styl'

#app
  height 100%
  overflow hidden
.app-bottom-tabber
  border-top 1px solid $color-theme
  background-color #fff
  box-shadow 0 0px 12px #dadada
  .is-selected
    background-color #fff
  a
    background-color #fff
</style>
